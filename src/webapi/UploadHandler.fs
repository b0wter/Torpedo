...
let requiresFormParameters (parameters: string seq) (addToContext: bool) : HttpHandler =
    fun (next: HttpFunc) (ctx: HttpContext) ->
        task {
            ...
            match requireParamterIn (findInFormItems ctx) ctx parameters addToContext with
...
let uploadWorkflow (_: string) : HttpHandler =
        ...
        task {
            return! (match ctx.Request.HasFormContentType with
                     ...
                     | true  -> 
                        ...
                        let path = ctx.Items.["folder"].ToString() |> Path.GetDirectoryName
                        ctx.Request.Form.Files
                        |> Seq.iter (fun file ->
                                         let newName = FileAccess.getUniqueFilename (Path.Combine(path, file.FileName))
                                         let stream = File.OpenWrite(newName)
...
let validateUploadToken (basePath: string) (onSuccess: HttpHandler) (onError: HttpHandler): HttpHandler =
    fun (next: HttpFunc) (ctx: HttpContext) ->
        task {
            ...
            let tokens = files
                         |> List.map (fun s -> s |> FileAccess.getTextContent |> TokenValueContent.AsTotal |> (deserializeToken s ""))
                         |> filterOks
                         
            let toSearch = string ctx.Items.["token"]
            let result = tokens |> Tokens.findTokenContainingValue toSearch
            match result with
            | None -> return! (onError next ctx)
            | Some token ->
